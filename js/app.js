window.formatLikersText = function(likeCount, recentLiker, secondLiker, showLinks, postId) {
    ...
    if (showLinks) {
        ...
        if (likeCount === 1) {
            ...
            return `Liked by ${recentLink} and <button type="button" onclick="window.event?.stopPropagation(); window.App.alert('Demo', 'Profile views are disabled in the demo.', 'info');" class="font-bold text-slate-700 dark:text-slate-300 hover:underline">${othersCount} other${othersCount > 1 ? 's' : ''}</button>`;
        }
    } else {
...
window.parseLinkAndPreviewForComments = function(text, idPrefix, id) {
    let parsedText = text || '';
    parsedText = parsedText ? parsedText.replace(/(^|\s)#(\w+)/g, `$1<a href="#" onclick="window.event?.stopPropagation(); window.App.alert('Demo', 'Search is disabled in the demo.', 'info');" class="text-blue-500 hover:underline">#$2</a>`).replace(/(^|\s)@(\w+)/g, `$1<a href="#" onclick="window.event?.stopPropagation(); window.App.alert('Demo', 'Profile views are disabled in the demo.', 'info');" class="text-blue-500 font-medium hover:underline">@$2</a>`) : '';
